...
static void Protanopia(float *Red, float *Green, float *Blue)
    __CPROVER_requires(__CPROVER_rw_ok(Red, sizeof(float)))
    __CPROVER_requires(__CPROVER_rw_ok(Green, sizeof(float)))
    __CPROVER_requires(__CPROVER_rw_ok(Blue, sizeof(float)))
    __CPROVER_requires(Red != Green && Red != Blue && Green != Blue)
...
void colourblind(cb_impairment Impairment, float *R, float *G, float *B)
    __CPROVER_requires(__CPROVER_rw_ok(R, sizeof(float)))
    __CPROVER_requires(__CPROVER_rw_ok(G, sizeof(float)))
    __CPROVER_requires(__CPROVER_rw_ok(B, sizeof(float)))
    __CPROVER_requires(R != G && R != B && G != B)
