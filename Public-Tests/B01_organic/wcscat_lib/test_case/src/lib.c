...
__CPROVER_requires(numElem >= 1 && numElem <= 3)
__CPROVER_requires(__CPROVER_is_fresh(dst, numElem * sizeof(wchar_t)))
__CPROVER_requires(src == ((void *)0) || __CPROVER_is_fresh(src, numElem * sizeof(wchar_t)))
...
__CPROVER_ensures(src == ((void *)0) ==> (__CPROVER_return_value == 22 && dst[0] == 0))
{
    wchar_t *ptr = dst;
    if (!dst || numElem == 0)
        return 22;
