...
static uint32_t get_bits(bs_t *bs, int n)
    ...
    __CPROVER_requires(bs->pos >= 0 && bs->pos <= 32)
...
int dequantize_granule(float *grbuf, bs_t *bs, L12_scale_info *sci,
    ...
    __CPROVER_requires(__CPROVER_is_fresh(bs->buf, 16))
    __CPROVER_requires(bs->pos >= 0 && bs->pos <= 16)
    __CPROVER_requires(bs->limit >= 0 && bs->limit <= 32)
    __CPROVER_requires(sci->total_bands >= 1 && sci->total_bands <= 2)
    __CPROVER_assigns(__CPROVER_object_whole(grbuf), __CPROVER_object_whole(bs))
